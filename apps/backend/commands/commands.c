    ...
    while(item_index < app->config->max_auctions_active){
        if(app->items[item_index].active == 1){
            ...
            if(pid_response != -1) {
                ...
                printf("        > Item %d: %s, BN: %d CV: %d CT: %s CB: %s Time: %d\n", app->items[item_index].unique_id, app->items[item_index].name, app->items[item_index].buy_now_value, app->items[item_index].current_value, app->items[item_index].category, app->items[item_index].bidder_name, app->items[item_index].duration);
            }
        }
        item_index++;
    }
